...
{
    // TODO: We should really be able to re-use timers!
    m_lock_hold_timer = AK::try_create<Timer>();
    if (!m_lock_hold_timer) {
        dbgln("Lock @ {:p} failed to create lock hold timer", this);
        return;
    }
    TimerQueue::the().add_timer_without_id(*m_lock_hold_timer, CLOCK_MONOTONIC, deadline, [this]() {
...
void Lock::did_remove_blocker(Thread::Blocker& b, void*)
{
    auto& blocker = static_cast<Thread::LockBlocker&>(b);
    switch (blocker.requested_mode()) {
    case Mode::Shared:
        VERIFY(m_waiting_shared > 0);
        m_waiting_shared--;
        break;
    case Mode::Exclusive:
        VERIFY(m_waiting_exclusive > 0);
        m_waiting_exclusive--;
        break;
    default:
        VERIFY_NOT_REACHED();
...
void Lock::did_unlock(Mode unlocked_mode)
{
    VERIFY(m_mode == Mode::Unlocked);
#if MEASURE_LOCK_TIME
    stop_locked_timer();
#endif
    auto next_mode = Mode::Unlocked;
    if (unlocked_mode == Mode::Exclusive) {
        if (m_waiting_shared > 0)
            next_mode = Mode::Shared;
        else if (m_waiting_exclusive > 0)
            next_mode = Mode::Exclusive;
    } else {
        if (m_waiting_exclusive > 0)
            next_mode = Mode::Exclusive;
        else if (m_waiting_shared > 0)
            next_mode = Mode::Shared;
    }
    if (next_mode == Mode::Unlocked)
        return;
    if (do_unblock([&](Thread::Blocker& b, void* data, bool& stop_iterating) {
        VERIFY(data);
        VERIFY(b.blocker_type() == Thread::Blocker::Type::Lock);
        auto& blocker = static_cast<Thread::LockBlocker&>(b);
        if (blocker.requested_mode() != next_mode)
            return false;
        auto requested_locks = blocker.requested_locks();
        if (!blocker.unblock())
            return false;
