...
class RemoteDesktopServerConnection final : public IPC::Connection<RemoteDesktopClientEndpoint, RemoteDesktopServerEndpoint, RemoteDesktopServerConnection, Core::TCPSocket>
...
public:
    RemoteDesktopServerConnection()
        : IPC::Connection<RemoteDesktopClientEndpoint, RemoteDesktopServerEndpoint, RemoteDesktopServerConnection, Core::TCPSocket>(*this, Core::TCPSocket::construct())
    ...
    virtual void associate_gfx_client(int gfx_client_id, u64 cookie) override
    {
        ...
        if (it == m_cookie_data.end()) {
            m_cookie_data.set(cookie, {
                gfx_client_id : gfx_client_id
            });
        } else {
    ...
    void handle_associate_window_client(int windowserver_client_id, u64 cookie)
    {
        ...
        if (it == m_cookie_data.end()) {
            m_cookie_data.set(cookie, {
                windowserver_client_id : windowserver_client_id
            });
        } else {
            auto& cookie_data = it->value;
            VERIFY(cookie_data.gfx_client_id > 0);
