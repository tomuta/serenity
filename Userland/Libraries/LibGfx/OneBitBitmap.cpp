...
{
    BitmapView bitmap(const_cast<u8*>(bytes.data()), bytes.size());
...
{
    auto bytes = ByteBuffer::create_zeroed(m_size.width() * m_size.height());
    BitmapView bitmap(bytes.data(), bytes.size());
    for (int y = 0; y < m_size.height(); y++) {
        for (int x = 0; x < m_size.width(); x++) {
            if (bit_at(x, y))
