...
BitmapData::BitmapData(Gfx::Bitmap const& bitmap, Gfx::IntRect const& rect)
    ...
    , m_bytes(ByteBuffer::create_uninitialized(Gfx::Bitmap::minimum_pitch(m_physical_rect.width(), bitmap.format()) * (size_t)m_physical_rect.height()))
{
...
BitmapDiff BitmapDiff::create(BitmapId id, Gfx::Bitmap const& original, Gfx::Bitmap const& changed, Gfx::DisjointRectSet const& change_rects)
{
    ...
    diff.bytes = ByteBuffer::create_zeroed(changed_tiles_bitmap_bytes + one_tile_bytes * tiles_count);
    BitmapView changed_tile_bitmap(diff.bytes.data(), tiles_count);
    ...
    auto changed_tiles = Gfx::Bitmap::try_create_wrapper(changed.format(), { max_tile_size, max_tile_size }, changed.scale(), pitch, changed_bitmap_data_ptr).release_nonnull();                                                                                           // TODO: GUI::BitmapView?
    auto changed_tiles_last_column = last_column_width == max_tile_size ? changed_tiles : Gfx::Bitmap::try_create_wrapper(changed.format(), { last_column_width, max_tile_size }, changed.scale(), last_column_physical_pitch, changed_bitmap_data_ptr).release_nonnull(); // TODO: GUI::BitmapView?
        ...
        for (int x = 0; x < tiles.width(); x++) {
            ...
            if ((change_rects.is_empty() || change_rects.intersects(tile_rect)) && !original.is_rect_equal(tile_rect, changed, tile_rect.location())) {
                ...
                if (changed_tiles_bitmap_bytes + (changed_bitmap_data_ptr - changed_bitmap_data_start_ptr) >= bytes_for_one_bitmap) {
                    ...
                    auto one_bitmap = Gfx::Bitmap::try_create_wrapper(changed.format(), bounds.size(), changed.scale(), pitch_for_one_bitmap, diff.bytes.data()).release_nonnull(); // TODO: GUI::BitmapView?
...
void BitmapDiff::apply_to_bitmap(Gfx::Bitmap& bitmap, Gfx::DisjointRectSet* changed_rects) const
{
    ...
    if ((u8)flags & (u8)DiffFlags::OneBitmap) {
        ...
        auto one_bitmap = Gfx::Bitmap::try_create_wrapper(bitmap.format(), bounds.size(), bitmap.scale(), pitch_for_one_bitmap, const_cast<u8*>(bytes.data())).release_nonnull(); // TODO: GUI::BitmapView?
    ...
    auto changed_tiles = Gfx::Bitmap::try_create_wrapper(bitmap.format(), { max_tile_size, max_tile_size }, bitmap.scale(), pitch, const_cast<u8*>(changed_bitmap_data_ptr)).release_nonnull();                                                                                           // TODO: GUI::BitmapView?
    auto changed_tiles_last_column = last_column_width == max_tile_size ? changed_tiles : Gfx::Bitmap::try_create_wrapper(bitmap.format(), { last_column_width, max_tile_size }, bitmap.scale(), last_column_physical_pitch, const_cast<u8*>(changed_bitmap_data_ptr)).release_nonnull(); // TODO: GUI::BitmapView?
    Gfx::Painter painter(bitmap);
    for (int y = 0; y < tiles.height(); y++) {
        for (int x = 0; x < tiles.width(); x++) {
...
PaletteData::PaletteData(Gfx::Palette const& palette)
    : m_bytes(ByteBuffer::create_uninitialized(palette.impl().internal_buffer().size()))
{
...
{
    auto anonymous_buffer = Core::AnonymousBuffer::create_with_size(m_bytes.size());
...
bool decode(Decoder& decoder, Gfx::Painter::LineStyle& line_style)
{
    u8 line_style_u8 = 0;
    if (!decoder.decode(line_style_u8))
        return false;
...
bool decode(Decoder& decoder, Gfx::Painter::DrawOp& draw_op)
{
    u8 draw_op_u8 = 0;
    if (!decoder.decode(draw_op_u8))
        return false;
...
bool decode(Decoder& decoder, Gfx::Orientation& orientation)
{
    u8 orientation_u8 = 0;
    if (!decoder.decode(orientation_u8))
        return false;
...
bool decode(Decoder& decoder, RemoteGfx::BitmapData& bitmap_data)
{
    if (!decoder.decode(bitmap_data.physical_rect()))
        return false;
    if (!decoder.decode(bitmap_data.bytes()))
        return false;
    return true;
}
...
bool decode(Decoder& decoder, RemoteGfx::BitmapDiff& bitmap_diff)
{
    if (!decoder.decode(bitmap_diff.id))
        return false;
    u8 flags_u8 = 0;
    if (!decoder.decode(flags_u8))
        return false;
    bitmap_diff.flags = (RemoteGfx::BitmapDiff::DiffFlags)flags_u8;
    if (!decoder.decode(bitmap_diff.location))
        return false;
    if (!decoder.decode(bitmap_diff.size))
        return false;
    if (!decoder.decode(bitmap_diff.bytes))
        return false;
    return true;
}
...
bool decode(Decoder& decoder, RemoteGfx::PaletteData& palette_data)
{
    if (!decoder.decode(palette_data.bytes()))
        return false;
    return true;
}
