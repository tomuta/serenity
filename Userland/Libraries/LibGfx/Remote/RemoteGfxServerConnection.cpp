...
void RemoteGfxServerConnection::create_font_and_send_if_needed(i32 id, Gfx::Font const& font)
{
    ...
    if (m_remote_fonts.find(digest) == m_remote_fonts.end()) {
        ...
        case Gfx::Font::Type::Bitmap:
            async_create_bitmap_font_from_data(id, ByteBuffer::copy(font.bytes())); // TODO: avoid copy
            break;
        case Gfx::Font::Type::Scaled:
            async_create_scalable_font_from_data(id, ByteBuffer::copy(font.bytes()), font.presentation_size()); // TODO: avoid copy
            break;
        default:
            VERIFY_NOT_REACHED();
        }
        ...
        switch (font.font_type()) {
        case Gfx::Font::Type::Bitmap:
            async_create_bitmap_font_from_digest(id, ByteBuffer::copy(digest.data, digest.data_length()));
            break;
        case Gfx::Font::Type::Scaled:
            async_create_scalable_font_from_digest(id, ByteBuffer::copy(digest.data, digest.data_length()), font.presentation_size());
            break;
        default:
            VERIFY_NOT_REACHED();
        }
