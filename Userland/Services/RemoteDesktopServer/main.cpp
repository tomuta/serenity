...
int main(int argc, char** argv)
{
    ...
    auto test_bitmap = Gfx::Bitmap::try_create(Gfx::BitmapFormat::BGRA8888, { 400, 300 }, 1);
    auto test_bitmap2 = Gfx::Bitmap::try_create(Gfx::BitmapFormat::BGRA8888, { 400, 300 }, 1);
    auto test_bitmap3 = Gfx::Bitmap::try_create(Gfx::BitmapFormat::BGRA8888, { 400, 300 }, 1);
    ...
    remote_gfx_server->on_ready_to_accept = [&] {
        auto client_socket = remote_gfx_server->accept();
        if (!client_socket) {
            dbgln("RemoteGfxServer: accept failed.");
            return;
        }
        [[maybe_unused]] auto client = adopt_ref(*new RemoteDesktopServer::GfxClient(client_socket.release_nonnull(), server));
    };
