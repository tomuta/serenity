    ...
    , m_remote_compositor_server(Core::LocalServer::construct())
{
    ...
    m_remote_compositor_server->on_ready_to_accept = [this] {
        auto client_socket = m_remote_compositor_server->accept();
        if (!client_socket) {
            dbgln("WindowServer: Remote Compositor accept failed.");
            return;
        }
        static int s_next_compositor_id = 0;
        int compositor_id = ++s_next_compositor_id;
        IPC::new_client_connection<RemoteCompositorClientConnection>(client_socket.release_nonnull(), compositor_id);
    };
